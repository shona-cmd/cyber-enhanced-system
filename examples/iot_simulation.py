...
def generate_iot_data(device_id: str, device_type: str) -> Dict[str, Any]:
    ...
    data = {"device_id": device_id, "timestamp": time.time(),
            ...
            "sensor_reading": (random.uniform(20.0, 30.0)
                             if "sensor" in device_type else None),
            "actuator_status": (random.choice(["on", "off"])
                               if "actuator" in device_type else None)}
    return data


def simulate_normal_traffic(framework: NaashonSecureIoT,
                            device_id: str,
                            device_type: str,
                            count: int = 10):
    ...
    for i in range(count):
        ...
        print(f"Normal packet {i+1}: Status={result['status']}, "
              f"Anomaly={result['anomaly_detected']}")
        time.sleep(0.1)


def simulate_anomalous_traffic(framework: NaashonSecureIoT,
    ...
    for i in range(count):
        ...
        print(f"Anomalous packet {i+1}: Status={result['status']},"
              f" Anomaly={result['anomaly_detected']},"
              f" Response={result['response_taken']}")
        time.sleep(0.1)
...
def demonstrate_device_registration(framework: NaashonSecureIoT):
    ...
    devices = [("sensor_001", "temperature_sensor"),
    ...
    for device_id, device_type in devices:
        ...
        print(
            f"Registration of {device_id} ({device_type}): "
...
def run_simulation():
    ...
    print("\n--- Evaluation Metrics ---")
    accuracy = ((normal_count - false_positives) + detected_anomalies) \
        / total_packets * 100
    precision = detected_anomalies / (detected_anomalies + false_positives) \
        if (detected_anomalies + false_positives) > 0 else 0
    recall = (detected_anomalies / anomaly_count) * 100 \
        if anomaly_count > 0 else 0
    response_efficiency = (response_actions / detected_anomalies) * 100 \
        if detected_anomalies > 0 else 0
